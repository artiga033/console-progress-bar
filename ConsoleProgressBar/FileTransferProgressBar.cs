namespace AaronLuna.ConsoleProgressBar
{
    ...
    public class FileTransferProgressBar : ConsoleProgressBar
    {
        ...
        string GetProgressBarText(double currentProgress)
        {
            ...
            var fileSizeInBytes = this.FileSizeToString(FileSizeInBytes);
            var padLength = fileSizeInBytes.Length;
            var bytesReceived = this.FileSizeToString(BytesReceived).PadLeft(padLength, '\u00a0');
        ...
        //Replace the original method from an external dll
        private string FileSizeToString(long fileSizeInBytes)
        {
            // mag is 0 for bytes, 1 for KB, 2, for MB, etc.
            int mag = (int)Math.Log(fileSizeInBytes, 1024);
            // 1L << (mag * 10) == 2 ^ (10 * mag)
            decimal adjustedSize = (decimal)fileSizeInBytes / (1L << (mag * 10));
            return string.Format("{0:n1} {1}",
        adjustedSize,
        Suffixes[mag]);
        }
